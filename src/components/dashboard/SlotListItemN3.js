...
export default function SlotListItemN3({number, latestActiveSlotNumber, data, buyLevel}) {
        ...
        <div className="ternary">
            {
                !data.isActive? <div className="binary-root matrix-root__nonactive">
                    ...
                    onClick={latestActiveSlotNumber + 1 == number? () => buyLevel(SlotPage.TYPES.n3, number) : null} title={t('buy')}></i>
            ...
            <div className="ternary-children">
                {
                    ...
                    data.nodes.map((value, index) => {
                        return !value.isActive? <div key={`slot-n3-${number}-${index}`} className="matrix-children__nonactive " /> :
                        <div key={`slot-n3-${number}-${index}`} className={SlotList.FUNCTIONS.getNodeClass(value.type)}>
